...
namespace o2Mate
{
    ...
    internal class Handler : ICompiler
    {
        ...
        public void Display(BindingList<DisplayElement> list, Node<ICompiler> node, bool forcedIndent, ref int indent)
        {
            ...
            list.Add(new DisplayElement(this.TypeName, "displayHandler", new object[] { this.handlerName, this.indent.ToString(), false }));
        }
        ...
        public void DisplayReadOnly(BindingList<DisplayElement> list, Node<ICompiler> node, bool forcedIndent, ref int indent)
        {
            ...
            list.Add(new DisplayElement(this.TypeName, "displayHandler", new object[] { this.handlerName, this.indent.ToString(), true }));
        }

        public void Update(System.Windows.Forms.HtmlElement elem)
        {
            Int32.TryParse(elem.GetAttribute("indent"), out this.indent);
            this.handlerName = this.GetElementByName(elem, "handler").InnerText;
        }
        ...
        public void Save(ICompilateur comp, System.Xml.XmlWriter writer, ref System.Windows.Forms.HtmlElement child)
        {
            ...
            writer.WriteAttributeString("name", this.handlerName);
            if (this.indent > 0)
        ...
        public void Parse(ICompilateur comp, System.Xml.XmlNode node)
        {
            ...
            this.handlerName = node.Attributes.GetNamedItem("name").Value;
        }
        ...
        public bool IsComputable(IProcess proc, out string reason)
        {
            ...
            {
                if (this.handlerName.Contains(Compilateur.ParamIdentifier.ToString() + param.FormalParameter))
                {
                    if (!param.IsComputable)
                    {
                        reason = String.Format("Le paramètre '{0:G}' n'est pas calculable et ne peut pas être remplacé dans '{1:G}'", param.FormalParameter, this.handlerName);
                        return false;
                    }
                    this.handlerName = this.handlerName.Replace(Compilateur.ParamIdentifier.ToString() + param.FormalParameter, param.EffectiveParameter);
                }
            }
            return true;
        }
