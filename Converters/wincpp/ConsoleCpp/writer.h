...
class writer {
public:
	writer() : fileName(wstring(L"")), numIndent(0), line(true) {}
	writer(wstring& f) : fileName(f), numIndent(0), line(true) {}
	writer(const writer& w) { this->copy(w); }

	writer& operator=(const writer& w) { return this->copy(w); }
...
private:
	...
	writer& copy(const writer& w) {

		this->reset();
		this->fileName.append(w.fileName.c_str());
		this->numIndent = w.numIndent;
		this->line = w.line;
